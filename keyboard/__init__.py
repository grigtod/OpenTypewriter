...
class _Wait:
    ...
    if given no parameters, blocks until `stop` is called.
    """
    def __init__(self):
        self._lock = _Event()

    def Set(self, hotkey=None, suppress=False, trigger_on_release=False):
        if hotkey:
            remove = add_hotkey(hotkey, lambda: self._lock.set(), suppress=suppress, trigger_on_release=trigger_on_release)
            self._lock.wait()
